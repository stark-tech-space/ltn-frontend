...
export default function IncomeGraph({ getGraphData }: { getGraphData: (data: any) => void }) {
  ...
  const [grossProfitData, setGrossProfitData] = useState<{ date: string; value: number }[]>([]);
  const [operatingExpenses, setOperatingExpenses] = useState<{ date: string; value: number }[]>([]);
  const [costOfGoodsSold, setCostOfGoodsSold] = useState<{ date: string; value: number }[]>([]);
  const [operatingIncome, setOperatingIncome] = useState<{ date: string; value: number }[]>([]);
  const [preTaxIncome, setPreTaxIncome] = useState<{ date: string; value: number }[]>([]);
  const [equityAttributableToOwnersOfParent, setEquityAttributableToOwnersOfParent] = useState<
    {
      date: string;
      value: number;
    }[]
  ...
  const fetchData = useCallback(async () => {
    ...
    if (rst?.status === 200) {
      ...
      setIncomeData(revenseData.map((item) => ({ date: item.date, value: item.value })));
      setGrossProfitData(grossProfitData.map((item) => ({ date: item.date, value: item.value })));
      setOperatingExpenses(
        operatingExpenses.map((item) => ({ date: item.date, value: item.value })),
      );
      setCostOfGoodsSold(costOfGoodsSold.map((item) => ({ date: item.date, value: item.value })));
      setOperatingIncome(operatingIncome.map((item) => ({ date: item.date, value: item.value })));
      setPreTaxIncome(preTaxIncome.map((item) => ({ date: item.date, value: item.value })));
      setEquityAttributableToOwnersOfParent(
        equityAttributableToOwnersOfParent.map((item) => ({ date: item.date, value: item.value })),
      );
    ...
    return {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: equityAttributableToOwnersOfParent.map((item) => item.value / 1000),
